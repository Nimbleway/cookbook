...
These files are **verbatim copies** of the canonical integration so the skill can install the
...
- `nimble_agent_run.sql` — copy of [`snowflake/udtf-data-feeds/nimble_agent_run.sql`](https://github.com/Nimbleway/cookbook/blob/main/snowflake/udtf-data-feeds/nimble_agent_run.sql)
  (the `NIMBLE_AGENT_RUN` UDTF — used for ingestion).
- `nimble_search.sql` — copy of [`snowflake/cortex-agent-tools/01_nimble_search.sql`](https://github.com/Nimbleway/cookbook/blob/main/snowflake/cortex-agent-tools/01_nimble_search.sql)
  (the `NIMBLE_SEARCH` scalar UDF — the agent's live-web tool).
...
**Expected integration version: `1.1.0`** — the version these bundled copies install (see
