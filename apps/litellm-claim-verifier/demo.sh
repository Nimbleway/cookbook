...
export LITELLM_MASTER_KEY="${LITELLM_MASTER_KEY:?set LITELLM_MASTER_KEY, or unset LITELLM_BASE_URL to skip the proxy}"
...
arch() {
  ...
  ALL THREE CALLS LEAVE THROUGH ONE GATEWAY
        ...
        extract ──┐
        search  ──┼──▶  litellm proxy :4111  ──▶  Anthropic Messages API
        judge   ──┘         │                └─▶  Nimble Search
                            │
     ...
     watch    http://127.0.0.1:4111/ui  →  Logs
...
case "${1:-arch}" in
  arch) arch ;;
  run)  exec "$PY" verify.py samples/sports_draft.md --no-cache ;;
