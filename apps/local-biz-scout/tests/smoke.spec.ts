...
test('smoke test - app loads and displays home page', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Local Business Scout' })).toBeVisible();
  await expect(page.getByText('Find Local Businesses')).toBeVisible();
