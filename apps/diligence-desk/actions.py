...
def email_body_html(content, company):
    assessment = content.get("overall_assessment") or {}
    verdict = (assessment.get("verdict") or "").replace("_", " ").upper()
    top_risks = "".join(
        f"<li><b>{(r.get('severity') or '').upper()}</b> - {r.get('risk')}</li>"
        for r in (content.get("risks") or [])[:3] if isinstance(r, dict))
    return (
        f"<h2>Diligence memo: {content.get('company_name') or company}</h2>"
        f"<p><b>Verdict: {verdict}</b></p>"
        f"<p>{assessment.get('rationale', '')}</p>"
