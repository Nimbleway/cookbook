...
Builds a situation report on a breaking event from official, national, and local reporting: what happened, what's confirmed versus disputed, and a timeline, keeping separate incidents apart instead of merging them. Ships configured for a United Airlines system outage.
...
| Output | `artifacts/breaking_event_tracking_agent__pattern_A.md` | `artifacts/breaking_event_tracking_agent__pattern_B.json` (content + trust) and `.md` |

Both patterns share the same task, skill, goals, and source guidance, so you can
compare their outputs directly. Each item carries: `development, status, confirmed_by, time, source_url`.
