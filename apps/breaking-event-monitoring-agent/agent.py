"""
Breaking Event and Situation Monitoring Agent
=============================================
...
AGENT_NAME = "breaking-event-monitoring"
...
AGENT_TASK = (
    "Build a current situation report on the United Airlines system outage "
    "that grounded flights nationwide. Explain what happened, which flights "
    "and operations were affected, what United and the FAA confirmed, and "
    "which details remain disputed. Provide a timeline, and if reporting "
    "refers to more than one separate ground-stop incident, distinguish "
    "them clearly. Rank by significance and cite the evidence."
)

SKILL = (
    "You are a breaking-situation analyst. Combine fragmented reporting "
    "from official sources, national news, and local outlets into one "
    "coherent account. Separate new information from repeated coverage, "
    "reconcile conflicting reports against stronger sources, and be "
    "explicit about what is confirmed versus unverified. If reports refer "
    "to more than one distinct incident, distinguish them rather than "
    "merging them. Record a timestamp and source for each update."
)

GOALS = [
    "Retrieve official, national, and local reporting",
    "Group reports around distinct developments",
    "Cross-check conflicting claims against stronger sources",
    "Produce a timeline of verified updates and a list of unresolved claims",
]

SOURCE_HINTS = (
    "Prefer agency and company statements and outlets closest to the event. "
    "Use national coverage to corroborate; exclude social platforms."
)

OUTPUT_FIELDS = ["development", "status", "confirmed_by", "time", "source_url"]

# A sample query for trying the Search API directly (not used by either pattern).
SEARCH_QUERY = "United Airlines system outage grounded flights nationwide"
...
def run_search_api_agent() -> str:
    ...
    out = OUTPUT_DIR / "breaking_event_tracking_agent__pattern_A.md"
...
def run_agent_api(effort: str = EFFORT, poll_every: int = 15, timeout: int = 1800) -> dict:
    ...
    json_path = OUTPUT_DIR / "breaking_event_tracking_agent__pattern_B.json"
    json_path.write_text(json.dumps(payload, indent=2, default=str), encoding="utf-8")

    output = payload.get("output", payload)
    content = output.get("content", "") if isinstance(output, dict) else ""
    md_path = OUTPUT_DIR / "breaking_event_tracking_agent__pattern_B.md"
...
if __name__ == "__main__":
    print("== Breaking Event and Situation Monitoring Agent ==")
