...
def _make_search_tool():
    ...
    def nimble_search(
        ...
        search_depth="lite":     metadata only (title, url, description). Use in the
                                 broaden phase to collect candidate company names cheaply.
...
_JUNK_MARKERS = (
    "duplicate", "same as", "dedupe", "placeholder", "pointer", "(see excluded",
    "legacy brand", "product line within", "product within", "rebrand of",
    "(now part of", "division of", "subsidiary of", "(product of", "(platform component",
)
...
def _clean(result: ScreeningResult) -> ScreeningResult:
    ...
    for idx, c in enumerate(result.candidates):
        ...
        if any(m in name.lower() for m in _JUNK_MARKERS):
