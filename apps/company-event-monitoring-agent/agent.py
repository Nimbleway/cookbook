...
AGENT_NAME = "company-event-monitoring"
...
AGENT_TASK = (
    "Research the most important developments related to OpenAI from the "
    "past 30 days. Cover product launches, partnerships, leadership "
    "changes, legal or regulatory developments, and major competitive "
    "moves. Rank the developments by significance and cite the underlying "
    "evidence."
)

SKILL = (
    "You are a company and industry monitoring analyst. Find material "
    "developments across product, partnerships, leadership, "
    "legal/regulatory, and competitive moves. Prefer primary sources "
    "(company newsroom, regulator sites, filings) and use reputable press "
    "to corroborate. Deduplicate repeats of the same underlying event, and "
    "record a date and a source URL for each."
)

GOALS = [
    "Identify material developments across the categories above",
    "Categorize each event by type and relevance",
    "Rank by significance with a short rationale",
    "Attach a date and source_url to every event",
]

SOURCE_HINTS = (
    "Prefer company announcements, regulatory sources, and reputable trade "
    "and news outlets. Favor primary pages over aggregators; exclude social "
    "platforms."
)

OUTPUT_FIELDS = ["event", "type", "relevance", "date", "significance", "source_url"]

# A sample query for trying the Search API directly (not used by either pattern).
SEARCH_QUERY = "OpenAI executive leadership change"
...
def run_search_api_agent() -> str:
    ...
    out = OUTPUT_DIR / "company_event_monitoring_agent__pattern_A.md"
...
def run_agent_api(effort: str = EFFORT, poll_every: int = 15, timeout: int = 1800) -> dict:
    ...
    json_path = OUTPUT_DIR / "company_event_monitoring_agent__pattern_B.json"
    json_path.write_text(json.dumps(payload, indent=2, default=str), encoding="utf-8")

    output = payload.get("output", payload)
    content = output.get("content", "") if isinstance(output, dict) else ""
    md_path = OUTPUT_DIR / "company_event_monitoring_agent__pattern_B.md"
...
if __name__ == "__main__":
    print("== Company and Industry Event Monitoring Agent ==")
