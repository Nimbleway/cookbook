...
HOW TO WORK - one focused pass per diligence dimension ({dims}):
  - For each dimension run a search scoped to the sources that fit it: the company's own
    ...
    for legal exposure.
  - Start with search_depth="standard" to find the right pages, then call the tool again
    with full_content=true and num_results<=4 on the two or three pages you will cite for
    that dimension.
  - Always use search_depth="standard" for these scoped passes. Never pass
    search_depth="lite" together with include_domains: the API drops the domain
    allow-list in lite mode and returns unrelated off-domain results.
