...
def _compact(results, query: str, full: bool, cap: int, scan: bool = False) -> list[dict]:
    ...
    for r in results or []:
        ...
        # scan pass / no content: title + url + description only, no empty "content" key
        out.append(item)
    return out
...
def _make_search_tool():
    ...
    def nimble_search(
        ...
        search_depth="lite":     accepted as a "scan" hint — the request is still sent to
                                 the API at "standard" depth, and the results are trimmed
                                 locally to title/url/description. Use it to scan for the
                                 candidate documents worth reading. Lite is never sent to
                                 the API because lite mode silently ignores
                                 include_domains and returns off-domain results.
        ...
        # "standard"), which would silently break the primary-source scoping this agent
        # depends on. A lite request is honoured as a local metadata-only trim instead,
        # so a scan costs the same context it always did without losing domain scoping.
